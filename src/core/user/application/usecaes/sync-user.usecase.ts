...
export class SyncUserUseCase {
  ...
  constructor(private readonly userPort: SyncUserPort) {}

  async execute(input: SyncUserInput): Promise<User> { // ✅ Correct return type
    try {
      ...
      if (user) {
        ...
        // Create new user
        this.logger.debug(`Creating new user with external ID: ${input.externalId}`);
